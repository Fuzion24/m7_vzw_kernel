...
void mmc_start_bkops(struct mmc_card *card)
{
	...
	if (!mmc_card_doing_bkops(card) && (mmc_card_check_bkops(card) || is_storage_encrypting)) {
		...
		/* if bkops_check_staus is set, need_bkops is determined by bkops status
		 * no matter urgent bkops is set or not
		 */
...
int mmc_can_discard(struct mmc_card *card)
{
	...
	/* support discard for Sandisk 19nm Extreme 02f11o eMMC */
...
#if SD_DEBOUNCE_DEBUG
	/**
	   *   If the SD slot detect change interval less than de-bounce time,
	   *   we consider it is HW issue. If it is more than de-bounce time,
	   *   we consider it is user's behavior.
	   *
	   *   @ detect_wq: work queue delayed time
	   *   @ irq_diff: SD slot detect change interva
	   */
	if (mmc_is_sd_host(host)) {
		...
		// Work queue delayed time is more than SW setting due to CPU is busy.
		if (wq_diff_time > SD_detect_debounce_time)
			pr_info("%s: %s wq diff time= %d ms\n", mmc_hostname(host), __func__, wq_diff_time);
		// The SD can't be rescanned succesfully due to SD power had been turned off.
...
int mmc_card_support_bkops(struct mmc_card *card)
{
	if (card && mmc_card_mmc(card) && (card->ext_csd.rev >= 5 && card->ext_csd.bkops) && (card->host->caps & MMC_CAP_NONREMOVABLE)) {
		...
		if (card->cid.manfid == 0x15) {
			...
			if (card->ext_csd.rev >= 6)
				return 1;
			/* 16GB Samsung 21nm VHX eMMC [MCP] */
			...
			/* 32GB Samsung 21nm VHX eMMC [Discrete] */
			...
			if (card->ext_csd.rev >= 6)
				return 1;
			/* 19nm SanDisk extreme 02y11o eMMC */
			if (card->ext_csd.sectors == 30777344 ||
				card->ext_csd.sectors == 61071360 ||
				card->ext_csd.sectors == 122142720 ) {
				if (!strcmp(card->ext_csd.fwrev, "02f11o"))
