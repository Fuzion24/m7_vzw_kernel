...
/* DIAG */
static char diag_clients[32];	    /*enabled DIAG clients- "diag[,diag_mdm,diag_qsc]" */
					...
					struct usb_configuration *c)
{
	...
	/* function diag is first diag port */
	if (diag_number < 1)
		return err;
					...
					struct usb_configuration *c)
{
	...
	/* if we found anyone, we should treat it as success */
	if (found)
		return 0;
	return err;
}
